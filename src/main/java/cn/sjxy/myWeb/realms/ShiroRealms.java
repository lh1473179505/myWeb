...
public class ShiroRealms extends AuthenticatingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		//System.out.println("从数据库获取的name："+name+"所对应的信息");
		//4.若用户不存在，抛出UnknownAccountException异常
		if(user==null) {
			throw new UnknownAccountException("用户不存在！");
		}
		//5.根据用户信息，抛出其他的异常AuthenticationException
		if("other".equals(name)) {
			throw new AuthenticationException("用户锁定");
		}
		...
		//盐值，通过ByteSource接口里的内部类Util的butes()方法实现，bytes()参数一般是惟一的
